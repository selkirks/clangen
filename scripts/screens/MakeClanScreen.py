...
class MakeClanScreen(Screens):
    ...
    def _get_cat_tooltip_string(self, cat: Cat):
        ...
        return f"<b>{cat.name}</b><br>{cat.get_genderalign_string()}<br>{i18n.t('general.' + cat.age, count=1)}<br>{i18n.t('cat.personality.' + cat.personality.trait)}<br>{cat.skills.skill_string(short=True)}<br>{cat.pelt.physical_trait_1}<br>{cat.pelt.physical_trait_2}<br>{cat.pelt.physical_trait_3}<br>{cat.pelt.physical_trait_4}"
